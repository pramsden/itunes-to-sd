...
public class LibraryParser implements ItunesLibraryParser {
	...
	private static final org.apache.log4j.Logger log = org.apache.log4j.Logger.getLogger(LibraryParser.class);
	...
	public ItunesLibraryParsedData parse() throws LibraryParseException, NoChildrenException {
		...
		Map<Long, Track[]> playlistTrackMap = new PlaylistTrackMapper(allPlaylists, allTracks).getPlaylistTracks();

		log.debug("Successfully Parsed.");
		return new ItunesLibraryParsedDataImpl(root, allPlaylists, allTracks, playlistTrackMap);
	}

	public void copyPlaylists(final String playlistName, final String destination, boolean analyzeDuplicates,
			ItunesLibraryParsedData itunesLibraryParsedData)
	...
	public void copyPlaylists(final String playlistName, final String destination,
			...
			final ProgressInformation<CopyTrackInformation> info, boolean analyzeDuplicates,
			ItunesLibraryParsedData itunesLibraryParsedData)
			throws IOException, FileCopyException, InvalidPlaylistException {
		for (final Playlist playlist : itunesLibraryParsedData.getAllPlaylists()) {
			if (playlist.getName().equalsIgnoreCase(playlistName)) {
				...
				if (informer != null && info != null) {
					copyPlaylists(playlist.getPlaylistId(), destination, informer, info, analyzeDuplicates,
							itunesLibraryParsedData);
				} else {
					copyPlaylists(playlist.getPlaylistId(), destination, analyzeDuplicates, itunesLibraryParsedData);
				}
	...
	public void copyPlaylists(final String playlistName, final String destination,
		...
		for (ProgressTracker progressTracker : progressTrackerList) {
			copyPlaylists(playlistName, destination, progressTracker.getProgressInformer(),
					progressTracker.getProgressInformation(), analyzeDuplicates, itunesLibraryParsedData);
		}
	...
	public void copyPlaylists(final Long playlistId, final String destination,
		...
		final File playlistFolder = new File(destination + File.separator + playlist.getName() + "-" + playlistId);
		...
		if (playlistFolder.exists()) {
			...
			final String[] srcArray = TrackLocationRetriever.getTrackLocations(plistTracks);
			...
			if (analyzeDuplicates) {
				...
				if (progressTrackerList != null) {
					copiedFiles = FileFetcher.copyFiles(
							srcArrayNoDuplicates.toArray(new String[srcArrayNoDuplicates.size()]),
				...
				if (progressTrackerList != null) {
					FileFetcher.copyFiles(srcArray, playlistFolder.getAbsolutePath(), progressTrackerList);
				} else {
					FileFetcher.copyFiles(srcArray, playlistFolder.getAbsolutePath());
				}
	...
	private String fetchDefaultDestination(final Long playlistId) {
		return PropertyManager.getConfigurationMap().get("playist.copy.destination") + File.separator + playlistId;
	}
	 ...
	 * @throws IOException
	 */
	public void copyAllPlaylists(final String destination, boolean analyzeDuplicates,
			ItunesLibraryParsedData itunesLibraryParsedData)
			throws IOException, FileCopyException, InvalidPlaylistException {
		Playlist[] playlists = itunesLibraryParsedData.getAllPlaylists();
		for (Playlist playlist : playlists) {
			final Long playlistId = playlist.getPlaylistId();
			copyPlaylists(playlistId, destination == null ? fetchDefaultDestination(playlistId) : destination,
					analyzeDuplicates, itunesLibraryParsedData);
		}
	}
}
