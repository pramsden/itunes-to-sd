...
public class Main {
	...
	private static void processLibraryParseOptions(Options options, CommandLine commandLine)
			throws LibraryParseException, NoChildrenException {
	...
	private static void processAppInquiryOptions(Options options, CommandLine commandLine) {
		if (commandLine.getOptions().length < 1 || commandLine.hasOption(CommandLineOptions.HELP.shortOptionName)) {
			printHelp(options);
		}
	...
	private static void printTracks(Track[] tracks) {
		...
		for (Track track : tracks) {
			...
			System.out.println(String.format("Id: %s; Name: %s, Location: %s", track.getTrackId(), track.getTrackName(),
					decodedLocation));
		}
	...
	private static void printPlaylists(Playlist[] playlists) {
		...
		for (Playlist playlist : playlists) {
			System.out.println(String.format("Playlist Name: %s; Number of Tracks: %s", playlist.getName(),
					playlist.getPlaylistItems().length));
		}
	...
	private static ItunesLibraryParsedData parseLibrary(ItunesLibraryParser itunesLibraryParser,
		...
		if (!commandLine.hasOption(CommandLineOptions.LIBRARY.shortOptionName)) {
			System.out.println(String.format(
					"Please specify the library option by either %s or %s followed by the full path to the library file in quotes.",
	...
	public enum CommandLineOptions {
		LIBRARY("lib", "library-file", true, "iTunes Music Library.xml file", true), PRINT_PLAY_LISTS("plists",
				"list-playlists", false, "Lists Playlists in the library",
				true), PRINT_TRACKS("trks", "list-tracks", false, "Lists Tracks in the library", true), HELP("h",
						"help", false, "Prints this help message",
						false), VERSION("v", "version", false, "Prints the version", false);
